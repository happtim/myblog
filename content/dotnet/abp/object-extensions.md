...
1. Domain.Shared 项目中 ModuleExtensionConfigurator 方法中，对需要扩展模块实体进行扩展。
2. Domain.Shared 项目中 ObjectExtendings 目录中，创建 如: UserConsts 方法进行字段的扩展。
3. 如果需要单独数据库字段。 需要在EntityFrameworkCore 项目中， EfCoreEntityExtensionMappings 方法中配置属性信息，并数据迁移。
