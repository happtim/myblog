...
`ApplicationDbContext`继承自`IdentityDbContext`，而`IdentityDbContext`又继承自`DBContext`。

`IdentityDbContext`包含`IdentityRole`、`IdentityRoleClaim`、`IdentityUser`、`IdentityUserClaim`、`IdentityUserLogin`、`IdentityUserRole`和`IdentityUserToken`实体的DbSet属性。您可以从Identity实体中找到这些实体的属性。
